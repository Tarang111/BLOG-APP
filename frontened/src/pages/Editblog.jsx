...
function Editblog() {
  ...
  const [blogdata, setblog] = useState({
    ...
    content: null,
    draft: false,
    tags: [] // ✅ ensures tags is always an array
  })
  ...
  async function fetchById() {
    try {
      const data = blog // using redux blog for now
      ...
      setblog({
        ...
        draft: data.draft,
        tags: data?.tags[0].split(",") || [] // ✅ ensures tags always exists
      })

      initializeEditor(parsedContent)
    } catch (error) {
      toast.error("CANT FIND BLOG")
      console.log(error)
    }
  }

  function deletetag(index) {
    const updated = blogdata.tags.filter((_, i) => i !== index)
    setblog(prev => ({ ...prev, tags: updated }))
  }
  ...
  async function Handlesubmit() {
    ...
    try {
      const res = await axios.patch(`${import.meta.env.VITE_BACKENED_URL}/blog/:${id}`, formdata, config)
      setblog(formdata)
    ...
    <div className="md:w-[80%] w:[100%] mx-auto mt-10 md:p-0 p-2 flex flex-col gap-5 ">
      ...
      <div className="flex md:flex-row flex-col w-full gap-5 ">
          ...
          <label htmlFor="image" className="aspect-video object-cover bg-slate-600 ">
            {blogdata.image ? (
              typeof blogdata.image === "string" ? (
                ...
                <img src={URL.createObjectURL(blogdata.image)} alt="blog" />
              )
            ) : (
              <div className="aspect-video bg-slate-600 flex justify-center h-[10] items-center">
                Select image
              </div>
            )}
          </label>
        ...
        <div className="flex flex-col md:w-[50%] w-[100%] gap-2">
          <label htmlFor="" className="text-4xl">Title:-</label>
          <input
            type="text"
          ...
          <input type="text" placeholder="Enter tag..." className=" border-2 h-10 p-2" onKeyDown={handleKeyDown} /><br />
          <div className=" flex gap-2 w-[100%] flex-wrap">
            {Array.isArray(blogdata?.tags) && blogdata.tags.map((item, index) => (
              <p key={index} className="border-2 p-3 w-fit rounded-2xl text-[15px] flex justify-center items-center gap-2 bg-black text-white font-bold">
                {item}
      ...
      <div className="w-full flex-col flex gap-3">
        ...
        <select id="" name="" className="border-2 h-10 p-2 cursor-pointer" onChange={(e) => { setblog((prev) => ({ ...prev, draft: e.target.value })) }}>
      ...
      <div className="w-full  flex mb-10 ">
        <button className="border-2 p-2 md:w-[20%] w-fit rounded-2xl text-2xl bg-black text-white font-bold " onClick={Handlesubmit}>Edit</button>
      </div>
    </div>
  )
}
